...
<br /> 
           <table class="table table-striped table-bordered table-hover">
              ...
              $sql = "SELECT id, name, user_type, actived_on from user";
              $result = $conn-> query($sql);
              if ($result-> num_rows > 0) {
                   while ($row = $result->fetch_assoc()) {
                        echo "<tr><td>". $row["id"] . "</td><td>". $row["name"]. "</td><td>". $row["username"]. "</td><td>". $row["Email"]. "</td><td>". "<button type='button' name='edit' class='btn btn-primary editbtn'>Edit</button> <button type='submit' name='delete' class='btn btn-danger'>Delete</button>".  "</td></tr>";
